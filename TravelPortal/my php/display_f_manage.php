...
$status = "Rejected by Finance Manager";
...
if($result){
	 echo '<script>leave(); function leave(){window.location="f_manager.php?msg=Data Updated";}</script>'; //header not works because it is faster than php and we get the include from script not from web page.
	}
}
...
$status = "Request for Infromation by Finance Manager";
$bb = mysqli_real_escape_string($con,$_POST["bb"]);
$sql = "UPDATE tour set `status` = '$status' WHERE  `id` = '$bb';";
$result = mysqli_query($con,$sql) or die('invalid');
if($result){
...
<form action="display_f_manage.php" name="go" method="post" > 
<tr>
 <th>Tour Purpose</th>
   <td colspan="3">
